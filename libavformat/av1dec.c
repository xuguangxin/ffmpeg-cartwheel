...
static int obu_probe(const AVProbeData *p)
{
    ...
    ret = read_obu(p->buf + cnt, FFMIN(p->buf_size - cnt, MAX_HEAD_SIZE), &obu_size, &type, &has_size_flag);
    if (ret < 0 || type != AV1_OBU_SEQUENCE_HEADER || obu_size != 0 || !has_size_flag)
        return 0;
    cnt += ret;
...
static int obu_read_data(AVFormatContext *s, AVPacket *pkt, int size)
{
    ...
    if (ret < 0) {
        av_log(s, AV_LOG_ERROR, "Failed to allocate packet for obu\n");
        return ret;
    }
    if (size <= c->len) {
        ...
        if (ret != left) {
            av_log(s, AV_LOG_ERROR, "Failed to read %d frome file\n", left);
            return ret;
        }
        c->len = 0;
    }
...
static const AVClass obu_demuxer_class = {
    .class_name = "AV1 OBU demuxer",
    .item_name  = av_default_item_name,
    .option     = obu_options,
    .version    = LIBAVUTIL_VERSION_INT,
};

AVInputFormat ff_av1_obu_demuxer = {
    .name           = "av1",
    .long_name      = NULL_IF_CONFIG_SMALL("AV1 OBU"),
